...
(() => {
    ...
    const state = {
        ...
        mobileSwipeSuppressClickUntil: 0,
    ...
    function resetTrackItems(trackObj) {
        ...
        trackObj.mobileSnapFocusItem = null;
        trackObj.mobileSnapFocusUntil = 0;
        ...
        trackObj.lastRenderedMobileSnapFocusActive = null;
    }
    ...
    function moveTrackByItem(trackObj, direction, duration = MOTION.carouselFocus, options = {}) {
        ...
        state.targetFocalX = window.innerWidth / 2;
        if (options.mobileFocus) {
            trackObj.mobileSnapFocusItem = targetItem;
            trackObj.mobileSnapFocusUntil = performance.now() + MOTION.mobileSnapFocus;
        }
        horizontalSpringTo(trackObj, getTrackCenterTarget(trackObj.element, targetItem), duration);
    }

    function isMobileSnapSwipeEnabled() {
        return window.matchMedia?.('(max-width: 760px)').matches || window.innerWidth <= 760;
    }

    function bindMobileSnapSwipe(trackObj) {
        const trackEl = trackObj?.element;
        if (!trackEl?.classList.contains('photo-carousel-track')) return;
        if (trackEl.dataset.mobileSwipeBound === 'true') return;

        trackEl.dataset.mobileSwipeBound = 'true';
        let gesture = null;

        trackEl.addEventListener('click', event => {
            if (Date.now() < state.mobileSwipeSuppressClickUntil) {
        ...
        trackEl.addEventListener('pointerdown', event => {
            if (!isMobileSnapSwipeEnabled()) return;
        ...
        trackEl.addEventListener('pointermove', event => {
            ...
            if (!gesture.lockedHorizontal && absX > 10 && absX > absY * 1.15) {
                gesture.lockedHorizontal = true;
            }

            if (gesture.lockedHorizontal) {
                event.preventDefault();
            }
        });
        ...
        const finishGesture = event => {
            ...
            const absX = Math.abs(deltaX);
            const absY = Math.abs(deltaY);
            const isSwipe = absX >= 44 && absX > absY * 1.2;

            if (isSwipe) {
                event.preventDefault();
                state.mobileSwipeSuppressClickUntil = Date.now() + 450;
                moveTrackByItem(trackObj, deltaX < 0 ? 1 : -1, MOTION.carouselButtonStep, { mobileFocus: true });
            }
        ...
        trackEl.addEventListener('pointerup', finishGesture);
    ...
    function updateTrackItems(trackObj) {
        ...
        const mobileSnapFocusActive = isMobileSnapSwipeEnabled()
            && now < (trackObj.mobileSnapFocusUntil || 0);
        let hintItem = null;
        ...
        trackObj.items.forEach(item => {
            ...
            const forceMobileFocus = mobileSnapFocusActive
                && isPhotoItem
                && item === trackObj.mobileSnapFocusItem;
            const effect = forceMobileFocus ? 1 : naturalEffect;
    ...
    function physicsLoop(currentTime) {
        ...
        state.tracks.forEach(trackObj => {
            ...
            const mobileSnapFocusActive = currentTime < (trackObj.mobileSnapFocusUntil || 0);
            const needsRender = trackObj.hScrollAnim
                || state.autoScrollSpeed !== 0
                || photoStateActive
                || mobileSnapFocusActive
                || trackObj.lastRenderedPhotoStateActive !== photoStateActive
                || trackObj.lastRenderedMobileSnapFocusActive !== mobileSnapFocusActive
